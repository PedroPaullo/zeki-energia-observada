...
try:
    ...
    c=service.choices(); period=st.sidebar.selectbox('Competência',c['periods'],index=len(c['periods'])-1)
    queue=service.queue(period)
    st.subheader('Fila de investigação')
    st.caption('Ordenação descritiva por aumento absoluto de afetações reportadas frente ao mês civil anterior. Ela prioriza leitura; não afirma causa.')
    st.dataframe(queue[:10],use_container_width=True,hide_index=True)
    byname={f"{x['display_name']} ({x['cnpj']})":x for x in c['distributors']}; dn=st.sidebar.selectbox('Distribuidora',list(byname)); d=byname[dn]
    groups=[x for x in c['conjuntos'] if x['cnpj']==d['cnpj']]; gm={f"{x['display_name']} ({x['conjunto']})":x for x in groups}; gn=st.sidebar.selectbox('Conjunto elétrico',list(gm)); g=gm[gn]
