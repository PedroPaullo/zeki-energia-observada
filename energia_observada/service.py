...
def build_dossier(cnpj,conjunto,period,municipio=None,context=True,data_dir=None):
 ...
 with _con(a,data_dir) as c:
  ...
  rrel='read_parquet('+literal((root_dir(data_dir)/a['model_path']).as_posix())+')'; raw=rows(c,f'SELECT * EXCLUDE (_eo_hash,_eo_key_valid,_eo_duplicate) FROM {rrel} WHERE _eo_cnpj=? AND _eo_conjunto=? AND _eo_period IN (?, ?) AND NOT _eo_duplicate ORDER BY _eo_period DESC, _eo_row LIMIT 100',[cnpj,conjunto,period,previous])
  return {'selection':{'cnpj':cnpj,'conjunto':conjunto,'period':period,'comparison_period':previous,'municipio':municipio},'current':current,'previous':prior,'history':history,'assessment':assessment,'source':a,'records_preview':raw,'records_preview_limit':100,'evidence_periods':[previous,period],'formulas':{'Afetações reportadas':'SUM(QtdConsumidoresAfetados válidos)','P90 duração':'quantile_cont((fim-início)/hora, 0.9)'},'limitations':['Aumento de interrupções não prova causa.','Município identifica o equipamento, não necessariamente consumidores afetados.','Ausência de dados não equivale a ausência de interrupções.','Não substitui análise regulatória ou técnica.'],'context':{'status':'descritivo' if context else 'desativado','notice':'Comparação descritiva; equivalência estatística não estabelecida.'}}
def export_dossier(dossier,data_dir=None,output_dir='exports'):
 ...
 changes=dossier['assessment']['changes']
 md='# Dossiê de Investigação\n\n'+'\n'.join(f"- {x['text']}" for x in dossier['assessment']['narrative'])+'\n\n## Seleção\n'+f"- Conjunto: {dossier['selection']['conjunto']}\n- Competência atual: {dossier['selection']['period']}\n- Competência comparada: {dossier['selection']['comparison_period']}\n\n## Variações observadas\n"+'\n'.join(f"- {name}: {item['current']} versus {item['previous']} ({item['percent']:.2f}%)" if item['percent'] is not None else f"- {name}: comparação percentual indisponível" for name,item in changes.items())+'\n\n## Limitações\n'+'\n'.join('- '+x for x in dossier['limitations'])
 with zipfile.ZipFile(path,'w',zipfile.ZIP_DEFLATED) as z:
  ...
  with _con(active,data_dir) as con, z.open('registros.csv','w') as binary:
   cursor=con.execute('SELECT * EXCLUDE (_eo_hash,_eo_key_valid,_eo_duplicate) FROM read_parquet(?) WHERE _eo_cnpj=? AND _eo_conjunto=? AND _eo_period IN (?, ?) AND NOT _eo_duplicate ORDER BY _eo_period, _eo_row',[str(model),selection['cnpj'],selection['conjunto'],selection['comparison_period'],selection['period']])
